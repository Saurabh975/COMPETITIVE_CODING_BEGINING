...
public class LongestIncreasingSubsequence {
    ...
    static void Blackrise() {                                                           //The name Blackrise is my pen name... you can change the name according to your wish........
        ...
        int count[]=new int[n];                             //initially all the array values were to be made 1. But it's better to finally ad 1 to the answer.

        LIS(ar,n,count);
        int max=MAX(count,n);
        pw.println(max+1);
        for(int i=0;i<n;i++)pw.print(count[i]+" ");
        te = Calendar.getInstance();
        te.setTime(new Date());
        pw.printf("\nExecution time was :- %f s\n", (te.getTimeInMillis() - ts.getTimeInMillis()) / 1000000.00);
    }

    static void LIS(int ar[],int n,int count[])
    {
        for(int i=1;i<n;i++)
            for(int j=0;j<=i;j++)
    ...
    static int MAX(int count[],int n)
    {
        int max=0;int z=0;
        for(int i=0;i<n;i++)
            if(count[i]>count[z]){max=count[i];z=i;}
            return max;
    }
}