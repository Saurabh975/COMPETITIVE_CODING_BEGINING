...
public class theMouseHunt {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        int ans = 0;
        for (int i = 1; i < cost.length; i++) cost[i] = ni();
        for(int i=1;i<=n;i++)
        {
            ar[i]=ni();
            vis[ar[i]]++;
        ...
        while (!q.isEmpty()) {
            ...
            vis[ar[node]]--;
            flag[node] = 1;
            if (vis[ar[node]] == 0) q.add(ar[node]);
        }

        for (int i = 1; i <= n; i++)
            if (flag[i]==0) ans += dfs(i, flag, cost, ar);
    ...
    static int dfs(int node, int flag[], int cost[], int ar[]) {
        flag[node] = 1;
        if (flag[ar[node]] == 1) return cost[node];
        return Math.min(cost[node], dfs(ar[node], flag, cost, ar));
