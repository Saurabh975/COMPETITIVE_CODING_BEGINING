...
public class Elections {
    ...
    static void Blackrise() {                                                       //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        ArrayList<Integer> ar[] = new ArrayList[5005];
        ...
        for (int i = 1; i <= n; i++) {
            ...
            for (int j = 2; j <= m; j++) {
                if (ar[j].size() >= i) {
                    for (int k = 0; k <= ar[j].size() - i; k++) {
                        ans += ar[j].get(k);
                        tot++;
                    }
                    for (int k = ar[j].size() - i + 1; k < ar[j].size(); k++) temp.add(ar[j].get(k));
                } else
                    for (int k = 0; k < ar[j].size(); k++) temp.add(ar[j].get(k));

            }
            Collections.sort(temp);
            if (tot < i)
                for (int k = 0; k < i - tot; k++)
                    ans += temp.get(k);
