...
//    static long iter=0;
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        for(int i=0;i<prime.size();i++)
        {
            ...
            {
                if(i==j)continue;
                if(prime.get(i)*prime.get(j)>200)break;
                else semiPrimes.add(prime.get(i)*prime.get(j));
            }
        }
        HashMap<Integer,Integer> hm=new HashMap<>();
        for(int i=0;i<semiPrimes.size();i++)
            for(int j=0;j<semiPrimes.size();j++)hm.put(semiPrimes.get(i)+semiPrimes.get(j),1);

        int t=ni();
        while(t-->0)
        {
            if(hm.containsKey(ni()))pl("YES");
            else pl("NO");
        }
