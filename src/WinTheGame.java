...
public class WinTheGame {
    ...
    static void Blackrise() {                                                           //The name Blackrise is my pen name... you can change the name according to your wish........
        ...
        while (t-- > 0) {
            ...
            ans = probablity((int) a, (int) b);
            pw.println(dm.format(ans));
        }
    ...
    public static double probablity(int r, int g) {
        if (r <= 0 || g <= 0) {
            return 1;
        }
        else {
            return (((double) r / (r + g)) + (g / (double) (r + g)) * ((g - 1) / (double) (r + g - 1)) * probablity(r, g - 2));
        }
    }
}