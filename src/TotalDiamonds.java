...
public class TotalDiamonds {
    ...
    static void Blackrise() {                                                           //The name Blackrise is my pen name... you can change the name according to your wish........
        ...
        ts.setTime(new Date());
        int size = (int) 1e6 + 5;
        long tot_dia[] = new long[size];
        long oddeve[] = new long[2 * size + 1];
        long sum_oddeve[] = new long[2 * size + 1];
        for (int i = 1; i < 2 * size + 1; i++) {
            oddeve[i] = sum(i);
            sum_oddeve[i] = sum_oddeve[i - 1] + oddeve[i];
        }
        tot_dia[1] = 2;

        for (int i = 2; i < size; i++)
            tot_dia[i] = tot_dia[i - 1] + 2 * (sum_oddeve[2 * i] - sum_oddeve[i]) - oddeve[2 * i];

        int t = input.nextInt();
        while (t-- > 0)
            pw.println(tot_dia[input.nextInt()]);
