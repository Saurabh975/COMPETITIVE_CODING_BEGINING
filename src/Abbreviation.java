...
public class Abbreviation {
    ...
    static void Blackrise() {     //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        while(t-->0) {
            ...
            boolean dp[][]=new boolean[1111][1111];
            dp[0][0]=true;
            for(int i=0;i<s1.length();i++)
            {
                for(int j=0;j<=s2.length();j++)
                {
                    ...
                    if(j<s2.length() && Character.toLowerCase(s1.charAt(i))==Character.toLowerCase(s2.charAt(j)))dp[i+1][j+1]=true;
                }
            }

            pl(dp[s1.length()][s2.length()]?"YES":"NO");
        }
