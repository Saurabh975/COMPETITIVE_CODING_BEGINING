...
public class Sumitsubarray {
    ...
    static void Blackrise() {                                                           //The name Blackrise is my pen name... you can change the name according to your wish........
        ...
        while (t-- > 0) {
            ...
            for (int i = 1; i < n + 1; i++) {
                int x = input.nextInt();
                sum1 = Math.max(sum1 + x, Math.max(sum1, x));
                ar[i] = ar[i - 1] + x;
            }
            long sum2[][] = new long[n + 1][n + 1];
            pw.println(subArrayMax(ar, sum2, 1, n) + " " + sum1);
        }
    ...
    static long subArrayMax(long ar[], long sum[][], int s, int e) {
        if (s > e) return Integer.MIN_VALUE;
        if (sum[s][e] != 0) return sum[s][e];
        sum[s][e] = ar[e] - ar[s - 1];
        return Math.max(sum[s][e], Math.max(subArrayMax(ar, sum, s + 1, e), subArrayMax(ar, sum, s, e - 1)));
    }
}
