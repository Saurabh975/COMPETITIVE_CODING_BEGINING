...
public class waitingInTheQueue {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        while (t-- > 0) {
            ...
            for(int i=0;i<n;i++)
            {
                people[i]=Math.max(0,(m+i-(ar[i]/l)));
            }
            int time[]=new int[n+1];int min=Integer.MAX_VALUE,temp;
            for (int i = 0; i < n; i++) {
                int x=(int)Math.ceil((double)(ar[i])/(double) (l))*l;
                if(x==ar[i])
                    temp=x+l;
                else temp=x;
                time[i]=((people[i])*l)+temp -ar[i];
                //pl(time[i]);
                if(time[i]<min && ar[i]<k)
                min=time[i];
                //pl(min);
            }
            time[n]=(n+1+m)*l-k;
            pl(Math.min(min,time[n]));
        }
