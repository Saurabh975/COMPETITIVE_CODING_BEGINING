...
public class vasyaAndMultisets {
    ...
    public static void main(String[] args) {                                 //threading has been used to increase the stack size.
        ...
        new Thread(null, null, "BlackRise", 1<<25)  //the last parameter is stack size which is desired, it is safe to chose 10^9 for all ques.
        {
            public void run() {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        int l=0;
        for(int i=1;i<105;i++)if(fre[i]==1)l++;int l2=l;
        if(l%2 ==0)
        {
            l/=2;
            for(int i=0;i<n && l>0;i++)
            {
                if(fre[ar[i]]==1){ch[i]='B';l--;}
            }
        }
        else
        {
            int e=-1;
            for(int i=0;i<105;i++)if(fre[i]>2){e=i;break;}

            if(e==-1){pl("NO");System.exit(0);}
            else
            {
                l2/=2;
                for(int i=0;i<n;i++)
                    ...
                    else if(fre[ar[i]]==1 && l2>0){ch[i]='B';l2--;}
            }
        }
        pl("YES");
        for(int i=0;i<n;i++)pw.print(ch[i]+"");
        pl();
