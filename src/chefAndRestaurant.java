...
public class chefAndRestaurant {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        while(t-->0)
        {
            ...
            int lastval=tm.get(tm.lastKey());
            while(m-->0)
            {
                int x=ni();
                if(x<firstKey){pl(firstKey-x);continue;}
                int ind=tm.floorKey(x);

                if(x>=lastval)pl(-1);
                else
                {
                    if(ind<=x && tm.get(ind)>x) pl(0);
                    else pl(tm.ceilingKey(x)-x);
                }
            }
        }
