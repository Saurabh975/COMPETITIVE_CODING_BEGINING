...
public class IPL {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        inp(ar);
        fees[0][1] = ar[0];
        fees[1][1] = Integer.max(ar[0], ar[1]);
        fees[0][2] = ar[0];
        fees[1][2] = ar[0] + ar[1];

        for (int i = 2; i < n; i++) {
            fees[i][0] = Integer.max(fees[i - 2][2], fees[i - 2][1]);
            fees[i][1] = Integer.max(fees[i - 1][0] + ar[i], Integer.max(fees[i - 2][1] + ar[i], fees[i - 2][2] + ar[i]));
            fees[i][2] = fees[i - 1][1] + ar[i];
        }
        int ans = -1;
        for (int i = 0; i < n; i++)
            for (int j = 0; j < 3; j++)
                ans = Integer.max(ans, fees[i][j]);
        pl(ans);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    static void inp(int ar[])
    {
        for(int i=0;i<ar.length;i++)ar[i]=ni();
    }
}
