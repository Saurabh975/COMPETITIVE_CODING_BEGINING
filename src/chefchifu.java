...
public class chefchifu {
    ...
    static void Blackrise() {
        ...
        while(t-->0)
        {
            ...
            ArrayList<Integer> size[]=new ArrayList[n];
            for(int i=0;i<n;i++)size[i]=new ArrayList<Integer>();
            //Arrays.fill(size,-1);
            for(int i=1;i<n;i++)
            {
                size[adl.get(i).size()].add(i);
            }
            int vis[]=new int[n+1];
            int c=0;
            for(int i=size.length-1;i>=0;i--) {
                if (size[i].size() != 0) {
                    Iterator<Integer> it = size[i].iterator();

                    while (it.hasNext()) {
                        int x=it.next();

                            vis[i] = 1;int temp=0;
                            Iterator<Integer> itr = adl.get(i).iterator();
                            while (itr.hasNext()) {
                                int z = itr.next();
                                if(vis[z]==0)temp=1;
                                vis[z] = 1;
