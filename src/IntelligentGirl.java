...
public class IntelligentGirl {
    ...
    static void Blackrise() {                                                       //The name Blackrise is my pen name... you can change the name according to your wish........
        ...
        ts.setTime(new Date());
        //long fac[] = new long[(int) 1e5 + 5];
        //factorial(fac,(int)1e5-1);
        String num = input.next();
        int ans = 0;
        int l = num.length();
        int ar[] = new int[l+1];
        counting(num, l-1, ar, ans);
        //pw.println(num);
        if(num.charAt(l-1)%2==0)ar[l]=1;
        for (int i = 1; i < l+1; i++) pw.append(ar[i] + " ");
        pw.flush();
        pw.println();
    ...
    static int counting(String num, int l, int ar[], int ans) {
        if (l == 0) {

            if ((int) num.charAt(0) % 2 == 0) ans++;
            //ar[0]=ans;
            return ans;
        }
        if((int)num.charAt(l)%2==0)ans++;

        ar[l] = counting(num, l - 1, ar, ans);
