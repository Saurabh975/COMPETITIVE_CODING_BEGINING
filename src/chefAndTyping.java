...
public class chefAndTyping {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        while (t-- > 0) {
            ...
            {
                String s=ns().trim();
                if(map.containsKey(s))time+=map.get(s)/2;
                else
                {
                    int temp=2;char ch=s.charAt(0);
                    for(int i=1;i<s.length();i++)
                    {
                        if(ch=='f'||ch=='d')
                        {
                            if(s.charAt(i)=='f'|| s.charAt(i)=='d')temp+=4;
                            else temp+=2;
                        }
                        else
                        {
                            if(s.charAt(i)=='f'|| s.charAt(i)=='d')temp+=2;
                            else temp+=4;
                        }
                        ch=s.charAt(i);
                    }
                    time+=temp;map.put(s,temp);
                }
            }
            pl(time);
        }
