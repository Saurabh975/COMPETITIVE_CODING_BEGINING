...
public class theRank {
    ...
    static void Blackrise() {                                                       //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        int n = ni();
        PriorityQueue<Pair> ar=new PriorityQueue<>(Comparator.reverseOrder());
        for(int i=1;i<=n;i++)
        {
            int marks=ni()+ni()+ni()+ni();
            ar.add(new Pair(marks,i));
        }

        for(int i=1;i<=n;i++)
        {
            if(ar.peek().y==1){pl(i);break;}
            ar.poll();
        }
    ...
    static class Pair implements Comparable<Pair>
    {
        ...
        public int compareTo(Pair other)   //method for comparing pairs, useful in sorting and...
        {
            ...
            return other.y-this.y;  //if x is same, the one with higher y is greater
        }
