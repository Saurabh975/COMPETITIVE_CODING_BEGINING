...
//    static long iter=0;
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        int q = ni();
        pair[] inp = new pair[n];
        for (int i = 0; i < n; i++) inp[i] = new pair(ni(), ni(), ni());
        int ar[] = new int[1000005];
        for (int i = 0; i < n - 1; i++) {
            for (int j = i + 1; j < n; j++) {
                khojo(inp[i], inp[j], ar);
            }
        }
        for (int i = 1; i < ar.length; i++) ar[i] += ar[i - 1];
        while (q-- > 0) {
            int k = ni();
            pl(ar[k]);
        }
    ...
    static void khojo(pair ob1, pair ob2, int ar[]) {
        double pro = Math.sqrt((ob1.n - ob2.n) * (ob1.n - ob2.n) + (ob1.m - ob2.m) * (ob1.m - ob2.m));
        double rad = ob1.r + ob2.r;
        if (pro >= rad) {
            // No overlap
            ar[(int) Math.floor(pro + rad + 1)]--;
            ar[(int) Math.ceil(pro - rad)]++;
        } else if (pro < Math.abs(ob1.r - ob2.r)) {
            // Inside
            ar[(int) Math.floor(pro + rad) + 1]--;
            ar[(int) Math.ceil(Math.max(ob1.r, ob2.r) - (pro + Math.min(ob1.r, ob2.r)))]++;
        } else if (pro == Math.abs(ob1.r - ob2.r)) {
            //Equal
            ar[0]++;
            ar[1]--;
