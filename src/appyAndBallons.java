...
public class appyAndBallons {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        balloncandy arr[] = new balloncandy[n];
        for (int i = 0; i < n; i++) arr[i] = new balloncandy(ar[i], nl());
        ...
        while (l <= r) {
            ...
            long temp = 0;
            answe = 0;
            for (balloncandy i : arr) {
                if (i.pro >= mid) {
                    long temp2 = (long) Math.ceil(((double) (i.pro - mid)) / ((double) i.m));
                    answe = (long) Math.max(answe, i.pro - temp2 * i.m);
                    temp += temp2;
                }
            }
            if (temp <= m) {
    ...
    static class balloncandy {
        long n;
        long m;
        long pro;

        balloncandy(long n, long m) {
            this.n = n;
            this.m = m;
            pro = multiply(this.m, this.n);
        }
    }
